...
double velocity_x(double v_x_prev, double v_y_prev, double y,
                  double delta_t);
double velocity_y(double v_x_prev, double v_y_prev, double y,
                  double delta_t);
...
int main(int argc, char *argv[]) {
    ...
    while (y >= 0.0) {
        ...
        v_x_new = velocity_x(v_x, v_y, y, delta_t);
        v_y_new = velocity_y(v_x, v_y, y, delta_t);
...
double velocity_x(double v_x_prev, double v_y_prev, double y,
    ...
    return v_x_prev - B2_m*v*v_x_prev*corr*delta_t;
}
...
double velocity_y(double v_x_prev, double v_y_prev, double y,
    ...
    double v = sqrt(v_x_prev*v_x_prev + v_y_prev*v_y_prev);
    double corr = exp(-y/y_d);
    return v_y_prev - g*delta_t - B2_m*v*v_y_prev*corr*delta_t;
}
